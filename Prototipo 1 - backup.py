...
class Objeto:
    ...
    def Colisao(self,objeto,P):
        
        if P==12:
            a=pygame.Rect((self.x,self.y+80),(self.xi,self.yi+80))
        else:
...
#hamburguer.y+=random.choice([0,-80])
#alface.y+=random.choice([0,-80])
                            ...
                            if pygame.mouse.get_pressed()[0]:
                                ...
                                while autores:
                                    ...
                                    if botaovoltar.ColisaoMouse():
                                        for event in pygame.event.get():
                                            botaovoltar.Texto("Voltar",pygame.color.Color("green"),30)
                                        if pygame.mouse.get_pressed()[0] or event.key == pygame.K_RETURN:
                                            autores = 0
                                    else:
                                        botaovoltar.Texto("Voltar",pygame.color.Color("black"),30)
    ...
    while not morto:
        ...
        if cnth==60 or cnth==30:            
            if random.choice([0,1]):                
                ...
                hamb.y+=random.choice([0,-100])
                listaf.append(hamb)                                     #Adiciona a nova comida na lista
            else:
                alfac=copy.copy(alface)                              #Cria um novo hamburguer
                alfac.x=display_width+random.choice(listarandom)         #Posição aleatória da comida
                alfac.y+=random.choice([0,-100])
                listaf.append(alfac)
